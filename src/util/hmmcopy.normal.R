# usage: Rscript HMMcopy.R --out_dir dir --normal normal.wig 
...
# read in gc.wig, ma.wig and normal wigs
...
# save results
write.table(somatic_segments$segs, file = 'normal_segments.txt', quote = FALSE, row.names = FALSE, sep = '\t');

# plot results
chrs <- c(1:22, 'X', 'Y');
for (i in 1:length(chrs)){
png (paste0 ('normal.chr', chrs[i], '.cnv.plot.png'), width = 980);
cols <- stateCols();
